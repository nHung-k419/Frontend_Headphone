...
const Product = () => {
  ...
  const { data: dataFilterProduct } = useQuery({
    queryKey: ["productFilter", checkActive, currentPage, KeySearch],
      ...
      <section className="grid lg:grid-cols-[1fr_3fr] grid-cols-1 max-w-7xl mx-auto mt-30">
        ...
        <div>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={`${currentPage}-${checkActive.idCategory}-${checkActive.idBrand}-${checkActive.valuePrice} -${KeySearch}`} // this is very important Framer Motion detect "trang đã đổi"
            ...
            >
              {dataFilterProduct?.products?.map((product, index) => (
                <div
                  key={product._id}
                  className="bg-white rounded-2xl my-4 shadow-lg lg:w-[300px] w-[330px] h-full cursor-pointer overflow-hidden"
                >
          ...
          <div className="w-full mt-5 mb-5">
            <div className="flex items-center justify-end lg:mr-0 mr-7 space-x-3">
              ...
              <span className="text-sm font-semibold">
                Page {currentPage} / {dataFilterProduct?.totalPages}
              </span>
              <button
