...
const Order_Items = () => {
  ...
  const { data } = useQuery({
    queryKey: ["order-items", idUser],
    queryFn: () => getOrderItems(idUser),
  });
  console.log(data?.OrderItemsProduct);
  ...
  return (
    <section className="lg:max-w-7xl mx-auto lg:w-full h-full mt-20 w-[380px] ">
      <div>
        ...
        {data?.OrderItemsProduct?.map((item) => (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center lg:space-x-20 space-x-7 lg:text-md text-sm">
                <div>
                  ...
                  <span className="font-semibold">{new Date(item.CreateAt).toLocaleDateString("vi-VN")}</span>
                </div>
                <div>
                  <h2 className="text-gray-400">Total Amount :</h2>
                  <span className="font-semibold">{(item.Id_Product?.Price * item.Quantity).toLocaleString("vi-VN", { style: "currency", currency: "VND" })}</span>
                </div>
                <div>
                  <h2 className="text-gray-400">Ship to :</h2>
                  <span className="font-semibold">{item?.Id_Order.Address}</span>
                </div>
              </div>
            </div>
