...
const CartSidebar = ({ isOpen, onClose }) => {
  ...
  // console.log(data?.resultCartItems,idUser);
  ...
  const HandlePreviousQuantity = (Id_Cart, Id_Product, Color) => {
    mutationPrevious.mutate({ Id_Cart, Id_Product, Color });
  };
  const HandleNextQuantity = (Id_Cart, Id_Product, Color) => {
    mutationNext.mutate({ Id_Cart, Id_Product, Color });
  };
  const handleDelete = (Id_Cart, Id_Product, Color) => {
    mutationDelete.mutate({ Id_Cart, Id_Product, Color });
  };
  return (
    <section className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-[3fr_1.5fr] grid-cols-1 h-full w-full select-none overflow-y-auto hide-scrollbar ">
      <div>
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-gray-500 mr-5 text-sm">{data?.resultCartItems?.length} ITEMS</span>
        </div>
            ...
            <div className="grid grid-cols-[1fr_4fr] space-y-5 space-x-4 pt-2 pl-2 pr-2 bg-white rounded-xl ">
              ...
              <div className="flex justify-between w-full relative">
                <div className="space-y-1">
                  <h1 className="text-lg text-gray-600">{item?.Id_Product?.Name}</h1>
                  <div className="flex items-center space-x-3">
                    <span className="text-sm text-gray-700 font-medium">${item?.Id_Product?.Price?.toLocaleString("vi-VN")} </span>
                  ...
                  <div className="flex mt-3 space-x-2 w-full ">
                    ...
                    <button className="rounded-md h-6.5 w-21 border-1 border-gray-300 flex items-center justify-center space-x-4 text-gray-500 ">
                      <span
                        onClick={() => HandlePreviousQuantity(item.Id_Cart, item?.Id_Product?._id, item.Color)}
                        className="cursor-pointer"
                      >
                        <LuMinus />
                      </span>
                      <span>{item.Quantity}</span>
                      <span onClick={() => HandleNextQuantity(item.Id_Cart, item?.Id_Product?._id, item.Color)} className="cursor-pointer">
                        <GoPlus />
                      </span>
                    </button>
                  </div>
                ...
                <div className="cursor-pointer">
                  <span onClick={() => handleDelete(item.Id_Cart, item?.Id_Product?._id, item.Color)}>
                    <IoCloseSharp />
                  </span>
                </div>
              </div>
      ...
      <div className="bg-gray-100 max-h-full h-full flex flex-col justify-between">
        ...
        <div className="lg:space-y-10 space-y-10 lg:mt-0 mt-5">
          ...
          <Link to={"/OrderConfirmation"} className="flex justify-center mr-5 ml-5">
            <button onClick={onClose} className="bg-black text-white w-73 h-12 flex items-center justify-center relative overflow-hidden group cursor-pointer mb-20">
